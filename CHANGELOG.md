...
- `signals` public read API now uses `SignalQueryService`, immutable read models and UoW-owned route boundaries, with `signals/services.py` reduced to legacy sync compatibility exports and class-first async query wrappers removed from the active surface.
