...
- `signals` now exposes HTTP only through a split read-only `api/` package with category routers for signals, decisions, market decisions, final signals, backtests and strategies, plus structured `404` API errors and a mode-aware builder; the legacy `signals/views.py` module is removed.
- `market_data` now exposes HTTP only through a split `api/` package with typed read/command/job routers, dedicated deps/errors/presenters modules, shared command execution helpers and a mode-aware router builder; the legacy `market_data/views.py` module is removed.
- `hypothesis_engine` now exposes HTTP only through a split `api/` package with dedicated read/command/job/stream routers, typed accepted job responses, a separate SSE stream adapter and a mode-aware router builder that keeps jobs/streams out of `ha_embedded`; the legacy `hypothesis_engine/views.py` module is removed.
- `patterns` now exposes HTTP only through a split `api/` package with separate read/command routers, dedicated deps/errors/presenters modules, typed API errors for not-found/validation branches and a mode-aware router builder; the legacy `patterns/views.py` module is removed.
- `indicators`, `portfolio`, `predictions` and `system` now also expose HTTP only through split `api/` packages with dedicated deps/presenters modules, mode-aware router builders and no legacy `views.py` entrypoints; the last active read-only HTTP modules have been cut over as well.
- HTTP/OpenAPI governance is now enforced in code: `FastAPI` uses centralized `operationId` generation from `core/http/router_policy.py`, `control_plane` routes carry category tags, and bootstrap tests assert unique operation ids plus `ha_addon`-specific schema exposure.
- Added deterministic OpenAPI snapshot tooling in `backend/scripts/export_openapi.py` and `core/http/openapi.py`, so schema export can now be driven per `mode/profile` and used later for CI-level diff control.
- Added committed OpenAPI snapshots, `backend/scripts/check_openapi.py` and root `make openapi-export-*` / `make openapi-check` targets, so full and `ha_addon` HTTP contracts can be diff-checked deterministically from the repo root.
...
- Added an autogenerated HTTP capability catalog in `docs/_generated/http-capability-catalog.md`, driven by `backend/scripts/export_http_capabilities.py` and `core/http/capabilities.py`, so operation-level API capabilities are now inspectable by `operationId`, method/path, category and mode availability.
- Added `backend/scripts/check_http_capabilities.py`, capability snapshot diff helpers and CI enforcement for `docs/_generated/http-capability-catalog.md`, so endpoint-level capability drift is checked automatically alongside OpenAPI and mode matrix artifacts.
- Enriched the generated HTTP capability catalog with typed metadata for `audience`, `execution_model`, `idempotency_policy`, `operation_resource_required` and `auth_policy`, so governance is now expressed as a real operation-level policy surface instead of route inventory only.
- Added a real operation resource model for async/job HTTP flows: queued triggers in `news`, `market_data`, `market_structure` and `hypothesis_engine` now return typed accepted responses with `operation_id`, Redis-backed lifecycle tracking lives in `core/http/operation_store.py`, and `system` now exposes `/api/v1/operations/{operation_id}`, `/result` and `/events` as the shared read surface for queued work.
- Added runtime idempotency/concurrency enforcement on the active HTTP mutation surface: repeated async/job triggers in `news`, `market_data`, `market_structure` and `hypothesis_engine` now deduplicate against the active tracked operation and return the same `operation_id` with `deduplicated=true`, while stale `control_plane` draft apply now returns a typed `409 concurrency_conflict` with structured version details.
- Added explicit consistency/freshness semantics for high-value analytical reads: `portfolio/state`, `market/radar`, `market/flow` and `coins/{symbol}/market-decision` now expose shared analytical metadata (`generated_at`, `consistency`, `freshness_class`, `staleness_ms`) instead of returning unqualified aggregate payloads.
- Added shared cache/revalidation governance for high-value analytical reads: `portfolio/state`, `market/radar`, `market/flow` and `coins/{symbol}/market-decision` now emit explicit `Cache-Control`/`ETag` headers, stable weak validators and deterministic `304 Not Modified` responses through `core/http/cache.py`.
...
- `news` and `hypothesis_engine` pipeline coverage now verifies worker/job outcomes through async sessions and query services instead of sync `SessionLocal` checks inside async tests.
- `anomalies`, `signals`, `cross_market` and `portfolio` worker/pipeline coverage no longer opens ad-hoc sync `SessionLocal` instances inside async tests; verification now reuses shared test sessions plus stream/event assertions.
...
- `market_structure` no longer commits inside `services.py`; write entrypoints now commit in views/tasks/tests, while source-health/snapshot side effects are queued through generic UoW after-commit hooks so event publication still happens strictly post-commit.
...
- `market_data` HTTP/task entrypoints no longer keep thin query/service wrapper helpers or the `AsyncSessionLocal = AsyncUnitOfWork` alias; views and TaskIQ jobs now instantiate `MarketDataQueryService`, `MarketDataService` and `MarketDataHistorySyncService` directly, with contract tests guarding that cleanup.
- `market_data` async services/query/repositories/events now share `market_data.support` for candle-config/event helpers, and the old sync `service_layer.py` DB facade plus its compatibility tests have been removed instead of being preserved behind deprecation wrappers.
- removed `market_data/repos.py` entirely; pure candle/timeframe contracts now live in `market_data.candles`, sync test-only seeding moved under `backend/tests/market_data_support.py`, and market-data write helpers no longer accept bare `AsyncSession` as an alternative mutation boundary.
...
- `indicators` no longer exposes thin `market_flow.py` / `market_radar.py` query wrappers or `IndicatorReadService`; callers and tests now use `IndicatorQueryService` / `IndicatorSignalRepository` directly.
...
- `anomalies` legacy selector exports are now tombstones; active callers read only through `AnomalyQueryService`.
...
- `predictions` low-level coverage now executes through `PredictionService` plus `SessionUnitOfWork` test harnesses, and `apps/predictions/engine.py` no longer contains direct DB access or transaction ownership.
...
- removed `signals/fusion.py` and `signals/history.py` entirely; callers and tests now use `SignalFusionService`, `SignalHistoryService`, `fusion_support.py` and `history_support.py` directly.
- `patterns` evaluation-job coverage now runs through async `PatternEvaluationService`, and the legacy sync `patterns/domain/evaluation.py` orchestration helper has been removed.
- removed the public sync `portfolio.selectors.py` query surface; callers and tests now read portfolio state/actions/positions only through `PortfolioQueryService`, while `selectors.py` remains as an empty compatibility tombstone with no public exports.
...
- removed the public sync `portfolio.engine` wrappers for state refresh, action evaluation and balance sync; internal callers now exercise async `PortfolioService` paths or helper-level engine functions directly.
- removed the remaining public `portfolio.engine` helper exports as well; tests now use `portfolio.support` and async `PortfolioService` private helpers, and `portfolio/engine.py` is reduced to an empty compatibility tombstone.
- removed the empty `portfolio.engine`, `portfolio.selectors`, `predictions.engine` and `anomalies.selectors` tombstone modules entirely, and persistence contract tests now assert those legacy import paths stay absent.
- removed the sync `predictions.selectors.py` layer; internal callers and tests now read through async `PredictionQueryService` only.
- removed the public sync `predictions.engine` wrappers for creation and pending evaluation; remaining legacy sync callers now use explicit internal engine impls during the final compatibility cutover, and `cross_market` no longer depends on the deprecated prediction wrapper API.
- removed `cross_market/engine.py` entirely; active callers and tests now execute through async `CrossMarketService`, `CrossMarketQueryService`, immutable read models and the existing `signals` runtime alignment path under UoW ownership.
- removed `patterns/selectors.py` entirely; callers and tests now import `signal_select()` directly from `patterns.query_builders` or use async `PatternQueryService`, `PatternAdminService` and `SignalQueryService`.
- removed the public sync `signals` read modules for backtests, strategies, decisions, market-decisions and final-signals; callers and tests now use async `SignalQueryService` only, and the last legacy `fusion.py` / `history.py` import paths are gone as well.
...
- `patterns.domain.regime` and `patterns.domain.scheduler` are now pure helper modules; their remaining sync DB helpers were removed and callers now read/write through `PatternQueryService` and `AnalysisSchedulerService` under the shared async persistence boundary.
- the sync-only `patterns.domain.engine.PatternEngine` compatibility path has been removed; runtime/bootstrap callers now rely exclusively on `PatternRealtimeService` and `PatternBootstrapService`.
- the sync-only `patterns.domain.clusters` and `patterns.domain.hierarchy` modules have been removed; meta-signal generation now exists only on `PatternRealtimeService` and contract tests assert the legacy modules are absent.
- `patterns.domain.cycle` now only exposes pure cycle classification helpers; cycle persistence/update flows run exclusively through `PatternRealtimeService` and `PatternMarketStructureService`.
- `patterns.domain.registry`, `discovery`, `narrative`, `context`, `decision`, `risk`, `strategy`, `statistics` and `success` are now pure helper/cache modules with no sync DB entrypoints; callers and tests use async task/query services or test-only seeding helpers instead.
- pattern-success validation is now cache-only on the active path; runtime/bootstrap services no longer pass dummy sync session handles into `patterns.domain.success`.
- `PatternStrategyService` now rewrites strategy rules through explicit delete/insert plus explicit `StrategyPerformance` loading, eliminating hidden lazy-load IO and the `MissingGreenlet` failure on async strategy refresh.
