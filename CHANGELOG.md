...
- `signals` legacy `fusion.py` and `history.py` modules now contain only deterministic helper exports; low-level coverage uses async services/repositories instead of session-driven sync query helpers.
- removed the public sync `signals` write wrappers from `fusion.py` and `history.py`; callers and tests now execute through `SignalFusionService` / `SignalHistoryService`, while those modules keep only pure helper logic shared by unit tests.
...
- removed the public sync `cross_market.engine` wrappers for relation refresh, sector refresh, leader detection and orchestration; active callers and tests now execute through async `CrossMarketService`, while `cross_market/engine.py` remains only for helper math/alignment logic still consumed by `signals`.
- removed the public sync `patterns.selectors.py` wrapper API; callers and tests now use async `PatternQueryService`, `PatternAdminService` and `SignalQueryService`, with `selectors.py` reduced to an internal `_signal_select` helper only.
- removed the public sync `signals` read modules for backtests, strategies, decisions, market-decisions and final-signals; callers and tests now use async `SignalQueryService` only, leaving residual sync compatibility only in `fusion.py` and `history.py`.
