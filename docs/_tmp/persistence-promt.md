...
Твоя задача — **внедрить единый стандарт persistence-архитектуры во весь проект** и последовательно перевести проект на:
...
# Важное допущение проекта

Проект находится в **активной разработке**. Для этой задачи действуют такие правила:

- **ломать внутренние контракты, сигнатуры и legacy-поверхности можно**, если это упрощает архитектуру;
- сохранять нужно прежде всего **бизнес-семантику, корректность use-case и целостность данных**, а не внутреннюю совместимость старых helper/service API;
- не нужно строить длинные compatibility layers, shim-слои, фасады и proxy-адаптеры ради плавной миграции;
- если старый слой мешает целевой архитектуре, нужно **переводить caller-ы и удалять его**, а не консервировать;
- временная совместимость допустима только как короткоживущая, явная и документированная мера;
- по умолчанию предпочтение отдаётся **прямой миграции на целевой async class-based контракт**.
- не делать import-правки ради самих import-правок: если меняется точка импорта или публичный модуль, нужно дойти до реальных caller-ов и поправить использование там, где это нужно.
...
- старые compatibility-пути удаляются по мере миграции и не становятся второй архитектурой.
...
- **breaking internal contracts is acceptable**
- **direct migration over compatibility wrappers**
...
- **no endless adapters / shims / facades**
...
- compatibility adapters / proxy-слои, которые только переадресуют вызов в новый service/repository без самостоятельной ценности;
- параллельное существование старого и нового persistence-path без чёткой причины и срока удаления.
- косметическая замена import path без фактической миграции caller-ов и удаления устаревшего слоя.
...
- это изолировано в repository/query service method или infrastructure helper с понятной ответственностью.
...
- удаление legacy persistence path или compatibility layer.
...
- все compatibility wrappers / shim-слои / legacy façade-модули.
...
- список business behavior to preserve;
- список legacy/internal contracts that may be intentionally broken or removed.
...
- anti-N+1 loading policy;
- logging policy;
- policy for deleting legacy persistence paths.
...
- при необходимости сразу удалить старые helper-обёртки вместо сохранения dual path.
...
- не сохранять legacy read facades, если caller-ы можно перевести прямо сейчас.
...
- если временно сохранён legacy-path, это должно быть видно в логах как исключение, а не норма.
...
- удалить legacy compatibility layers, если они больше не нужны caller-ам;
...
- repository methods;
- query services;
- behavior parity старого и нового запроса там, где сохраняется бизнес-семантика;
...
Если в ходе миграции ломается внутренний контракт:

- тестами фиксировать новый целевой контракт, а не тащить legacy-поведение без причины;
- compatibility coverage добавлять только если legacy-path сознательно оставлен как временное исключение.

---

# Обязательные требования к коммитам

После **каждого завершённого этапа или крупного доменного среза**:
...
- какие legacy-пути уже удалены и какие временные исключения ещё остались.
...
- строить многоэтажные compatibility adapters ради сохранения внутренних legacy-контрактов;
- оставлять shim/facade только потому, что "вдруг пригодится";
- менять import path точечно и оставлять старые import entrypoints живыми "на потом" без явного плана удаления;
...
- tests, proving parity and correctness там, где сохраняется бизнес-семантика;
- synced README / architecture docs / changelog;
- удалённые или минимизированные legacy persistence paths.
...
2. можно ломать внутренние контракты, если это упрощает архитектуру, но нельзя без причины ломать бизнес-семантику;
3. переиспользуй существующие подходящие абстракции;
4. не создавай параллельную архитектуру, dual path и compatibility-слои без реальной временной необходимости, и не ограничивайся переносом import-ов без фактической миграции использования;
5. объясняй trade-offs;
6. фиксируй изменения кодом, тестами, документацией и коммитом.

Главное:

**нужно не просто "добавить repo слой", а перевести IRIS на управляемую, типизированную, наблюдаемую и безопасную persistence-модель, даже если ради этого придётся удалять legacy-пути и ломать внутренние контракты.**
