...
12. Snapshot/check workflow, matrix drift control и capability catalog drift control уже привязаны к GitHub Actions; capability metadata enrichment для `audience/execution/idempotency/operation-resource/auth` тоже уже добавлен, а следующим слоем остаётся runtime-level operation resource model и более глубокая policy automation.
