...
- autogenerated HTTP availability matrix from real mode-aware OpenAPI assembly: `make api-matrix-export` -> `docs/_generated/http-availability-matrix.md`, plus `make api-matrix-check` for snapshot drift control
- autogenerated HTTP capability catalog from real OpenAPI `operationId` contracts: `make api-capabilities-export` -> `docs/_generated/http-capability-catalog.md`, plus `make api-capabilities-check` for snapshot drift control; catalog now also carries typed capability metadata for `audience`, `execution_model`, `idempotency_policy`, `operation_resource_required` and `auth_policy`
...
12. Snapshot/check workflow, matrix drift control и capability catalog drift control уже привязаны к GitHub Actions; capability metadata enrichment и runtime operation resource model для async/job flows уже добавлены, а следующим слоем остаётся runtime-level idempotency/concurrency enforcement и более глубокая policy automation.
