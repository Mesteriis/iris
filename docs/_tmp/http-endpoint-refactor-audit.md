...
- autogenerated HTTP capability catalog from real OpenAPI `operationId` contracts: `make api-capabilities-export` -> `docs/_generated/http-capability-catalog.md`, plus `make api-capabilities-check` for snapshot drift control
...
12. Snapshot/check workflow, matrix drift control и capability catalog drift control уже привязаны к GitHub Actions; следующим слоем при желании остаётся enrichment metadata для audience/idempotency/operation-resource policy.
