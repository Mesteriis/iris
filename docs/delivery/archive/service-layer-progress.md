...
> The refactor program documented here is complete and this file is archived for traceability.
...
- no `AsyncSession` or transport leak carried forward “to the next phase”
...
- build the canonical service/engine module that can be replicated across other domains
...
- close the main structural hotspots and math-heavy domains directly in their final form
...
- remove the remaining session, payload, and provider leaks and bring the service layer to the full target standard

## Progress Definition

The plan is considered complete only if:

- architecture-policy tests genuinely fail CI when the standard is violated
