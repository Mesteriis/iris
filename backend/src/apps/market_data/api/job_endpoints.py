...
from src.apps.market_data.api.contracts import CoinJobQueuedRead
...
from src.apps.market_data.api.presenters import coin_job_queued_read
...
@router.post(
    ...
    response_model=CoinJobQueuedRead,
...
async def run_coin_job_endpoint(
    ...
    if coin is None:
        ...
        raise http_error
    await dispatcher.dispatch_coin_history(symbol=coin.symbol, mode=mode, force=force)
    return coin_job_queued_read(symbol=coin.symbol, mode=mode, force=force)
