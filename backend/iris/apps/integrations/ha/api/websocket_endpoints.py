...
from iris.apps.integrations.ha.api.deps import HABridgeFacadeDep, HAWebSocketHubDep, StartedHABridgeRuntimeDep
from iris.apps.integrations.ha.bridge.websocket_hub import HASubscriptionState
from iris.apps.integrations.ha.errors import HACommandDispatchError
...
async def websocket_bridge(
    ...
    try:
        ...
        if hello.protocol_version != facade.instance().protocol_version:
        ...
        while True:
            ...
            if message_type == "command_execute":
                ...
                try:
                    dispatch = await facade.execute_command(
                        command=command_message.command,
                        payload=command_message.payload,
                    )
                except HACommandDispatchError as exc:
                    await websocket.send_json(
                        facade.command_dispatch_error_ack(request_id=command_message.request_id, error=exc)
                    )
                    continue
                await websocket.send_json(
                    facade.command_accepted_ack(
