...
from iris.apps.integrations.ha.schemas import HAHelloMessage, HASubscribeMessage, HAWelcomeMessage
...
class HAWebSocketHub:
    def __init__(self, facade: HABridgeFacade, *, max_queue_depth: int | None = None) -> None:
        self._facade = facade
    ...
    async def send_welcome(self, websocket: WebSocket, hello: HAHelloMessage) -> None:
        ...
        await websocket.send_json(
            HAWelcomeMessage(
                protocol_version=self._facade.instance().protocol_version,
                instance=self._facade.instance(),
                capabilities=self._facade.capabilities(),
            ).model_dump(mode="json")
        )

    async def send_initial_sync(self, websocket: WebSocket, subscription: HASubscriptionState) -> None:
        if subscription.entities:
            for message in await self._facade.entity_state_messages(sorted(subscription.entities)):
                await websocket.send_json(message)
        if subscription.collections:
            for message in await self._facade.collection_snapshot_messages(sorted(subscription.collections)):
                await websocket.send_json(message)
        await websocket.send_json(self._facade.system_health_message())
    ...
    def _mark_resync_required_locked(self, session: HASession, *, reason: str) -> None:
        ...
        session.queue.append(
            HAQueuedMessage(
                payload=self._facade.resync_required_message(
                    reason=reason,
