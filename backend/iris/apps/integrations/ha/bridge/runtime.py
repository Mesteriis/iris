...
from iris.apps.integrations.ha.bridge.event_consumer import create_ha_bridge_consumer, ensure_ha_bridge_group
from iris.apps.integrations.ha.bridge.websocket_hub import HAWebSocketHub
from iris.runtime.streams.consumer import EventConsumer
...
class HABridgeRuntime:
    def __init__(
        self,
        *,
        facade: HABridgeFacade | None = None,
        websocket_queue_depth: int | None = None,
    ) -> None:
        self.facade = facade or HABridgeFacade()
        self.hub = HAWebSocketHub(self.facade, max_queue_depth=websocket_queue_depth)
    ...
    async def handle_event(self, event: IrisEvent) -> None:
        messages = await self.facade.apply_event(event)
    ...
    async def _emit_tracked_operation_update(self, operation_id: str) -> None:
        ...
        if tracked is None:
            return
        message = await self.facade.operation_update_message(
