...
from iris.apps.explanations.contracts import (
    ExplainKind,
    ExplanationArtifactResult,
    ExplanationGenerationResult,
    ExplanationGenerationStatus,
)
from iris.apps.explanations.language import resolve_effective_language
...
from iris.apps.explanations.services.generation_service import ExplanationGenerationService
...
class ExplanationService(PersistenceComponent):
    ...
    async def _load_context_bundle(self, *, explain_kind: ExplainKind, subject_id: int) -> ExplanationContextBundle:
        if explain_kind is ExplainKind.SIGNAL:
            ...
            raise ValidationFailedError(params={"field": "explain_kind", "value": explain_kind.value})
        if bundle is None:
            raise ResourceNotFoundError(resource=explain_kind.value)
        return bundle
