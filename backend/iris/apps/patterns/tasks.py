...
from iris.core.db.persistence import to_jsonable_data
...
def _task_payload(result: object) -> dict[str, object]:
    payload = to_jsonable_data(result)
    if not isinstance(payload, dict):
        raise TypeError(f"Pattern task payload must be a dict, got {type(payload).__name__}")
    feature_snapshot = payload.pop("feature_snapshot", None)
    if feature_snapshot is not None and "_feature_snapshot" not in payload:
        payload["_feature_snapshot"] = feature_snapshot
