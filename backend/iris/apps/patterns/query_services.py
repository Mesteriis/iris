...
from iris.apps.cross_market.models import Sector, SectorMetric
...
from iris.apps.patterns.domain.regime import RegimeRead, detect_market_regime, read_regime_details
...
from iris.apps.patterns.query_builders import pattern_signal_ordering as _pattern_signal_ordering
from iris.apps.patterns.query_builders import signal_select as _signal_select
from iris.apps.patterns.read_models import (
    CoinRegimeReadModel,
...
class PatternQueryService(AsyncQueryService):
    ...
    async def count_coin_pattern_history(self, *, coin_id: int) -> int:
        return int(
            (
                await self.session.execute(
                    select(func.count())
