...
from iris.apps.hypothesis_engine.constants import DEFAULT_HYPOTHESIS_HORIZON_MIN, DEFAULT_TARGET_MOVE
...
def build_hypothesis_degraded_output(context: dict[str, Any]) -> dict[str, Any]:
    event_type = str(context.get("event_type", "unknown"))
    payload = dict(context.get("payload") or {})
    symbol = str(context.get("symbol") or payload.get("symbol") or f"coin-{int(context.get('coin_id', 0))}")
    ...
    return {
        ...
        "horizon_min": int(context.get("horizon_min") or payload.get("horizon_min") or DEFAULT_HYPOTHESIS_HORIZON_MIN),
        "direction": direction,
        "target_move": float(context.get("target_move") or payload.get("target_move") or DEFAULT_TARGET_MOVE),
...
__all__ = ["build_hypothesis_degraded_output"]
