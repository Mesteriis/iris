...
def test_service_layer_scorecard_builds_current_domain_snapshot() -> None:
    ...
    assert all(row.total_violations == 0 for row in rows)
    assert "signals" in by_domain
    assert "patterns" in by_domain
    assert "portfolio" in by_domain
    assert by_domain["patterns"].cutover_wave == "2"
    assert by_domain["patterns"].status == "clean"
...
def test_service_layer_scorecard_renders_markdown_and_json() -> None:
    ...
    assert payload["summary"]["outstanding_policy_violations"] == 0
